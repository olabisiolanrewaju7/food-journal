...
npx cap sync ios     # Re-link native plugins into the Xcode project after changing capacitor.config.ts or installing a @capacitor/* package
```
...
- `ios/App/App.xcodeproj` wraps the app as an iOS shell (bundle ID `com.ruach.foodjournal`), but **`capacitor.config.ts` points `server.url` at the live Vercel deployment** — the native WKWebView loads web content over the network from production, not from a local build. This means: (1) any change to `app/`, `components/`, or `lib/` only takes effect on-device after it's pushed and deployed to Vercel — an Xcode rebuild alone does nothing for web-layer changes; (2) an Xcode rebuild *is* required only when a native plugin changes (new `@capacitor/*` package, `capacitor.config.ts` edits, `Info.plist`/entitlements) — run `npx cap sync ios` first, then rebuild
- No CocoaPods — native dependencies resolve via Swift Package Manager (`ios/App/CapApp-SPM/Package.swift`, auto-generated by `cap sync`)
- Installed native plugins: `@capacitor/local-notifications`, `@capacitor/push-notifications`, `@capacitor/splash-screen` (config in `capacitor.config.ts` under `plugins.SplashScreen`)
- Client code that calls a Capacitor plugin must dynamically `import()` it and guard with `Capacitor.isNativePlatform()` — these plugins no-op or throw in a browser/PWA context (see `/settings/notifications` for the pattern)
- Every page's top header div needs the `.safe-area-pt` class (`app/globals.css`) so content clears the iOS status bar/notch; `.safe-area-pb` exists for bottom-edge cases
- `public/manifest.json` icons serve Android/PWA "Add to Home Screen"; `app/layout.tsx`'s `metadata.icons.apple` serves iOS Safari/PWA; the Xcode `AppIcon.appiconset` (App Store icon) is separate again and must be updated manually — none of these three share a source of truth

### Tab Shell (Persistent Navigation)

- `components/TabShell.tsx` mounts all 5 main tab pages (Home, History, Cravings, Coach, Settings) simultaneously and toggles visibility with CSS `display`, so switching tabs is instant with no re-fetch/remount — this is why `useNav`/`useTab` state can leak across tabs if not reset explicitly
- `lib/TabContext.tsx` tracks `activeTab` and `mountedTabs` (tabs get added to `mountedTabs` on first visit and stay mounted); `switchTab()` updates state *and* calls `router.push()` to keep the URL in sync
- `BottomNav` calls `switchTab()`, not `<Link>`, to drive this
- Auth pages (`/login`, `/register`, `/splash`, `/forgot-password`, `/reset-password`, `/privacy`) are listed in `TabShell.tsx`'s `AUTH_PREFIXES` array and bypass the tab-mounting logic entirely, rendering standalone — **this list must be kept in sync with `middleware.ts`'s public-route matcher**; a route that's public in middleware but missing from `AUTH_PREFIXES` will still silently mount all 5 authenticated tabs (and their API calls) hidden behind it
- Non-tab sub-pages (e.g. `/settings/goals`, `/settings/notifications`) fall through to a `{children}` render inside the same tree
- `components/Providers.tsx` nesting order is `SessionProvider > TabProvider > NavProvider`
...
- Hidden on `/login`, `/register`, `/splash`, `/forgot-password`, `/reset-password`, `/privacy` (`BottomNav.tsx`'s `HIDE_ON` array — keep in sync with `middleware.ts` and `TabShell.tsx`'s `AUTH_PREFIXES`); also hidden during photo preview and analysis via `NavContext` (`lib/NavContext.tsx`)
